...
class OpenApiConfig {
    ...
    public OpenAPI customOpenAPI(@Value("0.5") String appVersion) {
        return new OpenAPI()
                .components(new Components().addSecuritySchemes("jwtToken", new SecurityScheme().type(SecurityScheme.Type.OAUTH2).scheme("bearer").bearerFormat("JWT"))
                ...
                .info(new Info()
                        .title("Innomotors API")
                        .version(appVersion)
                        .description("This is a sample server Innomotors server.")
